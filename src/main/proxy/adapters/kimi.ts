...
const KIMI_API_BASE = 'https://www.kimi.ai'
...
interface ChatCompletionRequest {
  ...
  /** OpenAI-style reasoning effort level ('low' | 'medium' | 'high'), also implies thinking */
...
export class KimiAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; conversationId: string }> {
    ...
    // An explicit reasoning effort level also enables thinking mode
    if (!enableThinking && typeof request.reasoningEffort === 'string' && request.reasoningEffort) {
      enableThinking = true
    }
  ...
  static isKimiProvider(provider: Provider): boolean {
    return (
      provider.id === 'kimi' ||
      provider.apiEndpoint.includes('kimi.ai') ||
      provider.apiEndpoint.includes('kimi.com')
    )
  }
}
