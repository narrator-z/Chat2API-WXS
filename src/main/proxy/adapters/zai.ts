...
export class ZaiAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; chatId: string; requestId: string }> {
    ...
    // - GLM-5.3-Flash, GLM-5.3, GLM-5.1 and GLM-5-Turbo keep uppercase
    // - GLM-5V-Turbo uses lowercase "v" in the request model id
    // - GLM-5.2, GLM-5 and GLM-4.7 use lowercase request model ids
    ...
    // The official web client sends a reasoning effort level ("low" | "high" | "max",
    // default "max") alongside enable_thinking. Map the OpenAI-style value onto it.
    if (enableThinking && typeof request.reasoning_effort === 'string') {
      const level = request.reasoning_effort.toLowerCase()
      if (level === 'low' || level === 'high' || level === 'max' || level === 'medium') {
        features.reasoning_effort = level === 'medium' ? 'high' : level
      }
    }
